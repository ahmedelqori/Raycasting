...
/*   Updated: 2024/10/28 21:13:31 by ael-qori         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	flood_fill_neighbor(char **line, int i, int j)
{
	...
	error = (flood_fill_space(line, i + 1, j) && flood_fill_space(line, i - 1,
				j) && flood_fill_space(line, i, j + 1) && flood_fill_space(line,
				i, j - 1));
	if (is_door(line[i][j]))
		...
		t_parsing_helper *helper)
{
	...
	while (line[i][j])
	{
		if (!ft_check_entity(line[i][j]) || ((is_empty_space(line[i][j])
					|| is_door(line[i][j]) || is_orientation(line[i][j]))
